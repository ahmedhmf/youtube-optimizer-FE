...
import { AIConfigurationSettings } from '../../../../models/language.model';
import { Audits } from '../../../../stores/video-audits.store';
...
export class AnalyzeUrl {
  ...
  readonly activeTab = signal<TabType['id']>('url');
  ...
  private readonly MAX_FILE_SIZE = 200 * 1024 * 1024; // 200MB to match backend
  ...
  setActiveTab(tabId: TabType['id']): void {
    this.clearError();
  ...
  onFileSelected(event: Event): void {
    ...
    try {
      ...
      if (!this.isValidFileSize(file)) {
        this.setError({
          message: `File size too large. Maximum allowed size is ${this.MAX_FILE_SIZE / (1024 * 1024)}MB. Your file is ${(file.size / (1024 * 1024)).toFixed(2)}MB`,
  ...
  private containsSuspiciousContent(text: string): boolean {
    ...
    if (repeatedCharsRegex.test(text)) {return true;}
